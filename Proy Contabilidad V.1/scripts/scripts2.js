...
/**
 * ContApp — Sistema Contable Modular
 * Estructura limpia, abierta y con soporte para Mayor Auxiliar en tiempo real.
 */
...
const ContCalcular = {
  ...
  // NUEVO: Agrupa los asientos para obtener los totales por cuenta para el Mayor Auxiliar
  obtenerTotalesPorCuenta: function() {
    const resumen = {};

    ContApp.asientos.forEach(asiento => {
      // Si la cuenta no existe en nuestro resumen, la inicializamos en cero
      if (!resumen[asiento.cuenta]) {
        resumen[asiento.cuenta] = { debe: 0, haber: 0 };
      }
      
      // Sumamos al Debe o al Haber según corresponda
    ...
    return resumen; // Devuelve un objeto con las cuentas ordenadas y sus totales
  }
};

// 3. ENRUTADOR Y CONTROLADOR DE VISTAS
document.addEventListener('DOMContentLoaded', () => {
...
// 4. LÓGICA EXCLUSIVA DEL LIBRO DIARIO
const ModuloLibroDiario = {
  ...
  renderizarTodo: function() {
    // A. Renderizar Tabla del Libro Diario
    ...
    // B. NUEVO: Renderizar Tabla del Mayor Auxiliar (Derecha)
    ...
    if (tbodyMayor) {
      ...
      // Recorremos el objeto de cuentas agrupadas para dibujarlas
...
// 5. CONTROLADOR DE BLOQUEO Y NAVEGACIÓN
    ...
    enlaces.forEach(enlace => {
      ...
      if (destino !== 'libros.html' && !cuadrado) {
        ...
        enlace.onclick = function(e) {
          e.preventDefault();
          alert('Acceso restringido: Debes registrar asientos y cuadrar perfectamente el Total Debe y Total Haber en el Libro Diario antes de proceder.');
          return false;
        };
      } else {
